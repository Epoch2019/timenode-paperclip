...
function SupportMark({ supported }: { supported: boolean }) {
  ...
  return supported ? (
    <span className="inline-flex items-center gap-1 text-green-700 dark:text-green-400">
      ...
      {t("pages.companyEnvironments.supportYes", { defaultValue: "Yes" })}
    </span>
  ) : (
    <span className="text-muted-foreground">{t("pages.companyEnvironments.supportNo", { defaultValue: "No" })}</span>
  );
}
...
export function CompanyEnvironments() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("pages.companyEnvironments.breadcrumbCompany", { defaultValue: "Company" }), href: "/dashboard" },
    ...
    return (
      <div className="max-w-3xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("pages.companyEnvironments.heading", { defaultValue: "Company Environments" })}</h1>
        </div>
        <div className="rounded-md border border-border px-4 py-4 text-sm text-muted-foreground">
          {t("pages.companyEnvironments.enablePrompt", { defaultValue: "Enable Environments in instance experimental settings to manage company execution targets." })}
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6" data-testid="company-settings-environments-section">
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("pages.companyEnvironments.intro", { defaultValue: "Define reusable execution targets for projects, task workspaces, and remote-capable adapters." })}
        </p>
      </div>

      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("pages.companyEnvironments.matrixNote", { defaultValue: "Environment choices use the same adapter support matrix as agent defaults. SSH is always available for remote-managed adapters, and sandbox environments appear only when a run-capable sandbox provider plugin is installed." })}
        </div>
        {sandboxCreationEnabled ? (
          <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
            {t("pages.companyEnvironments.installedProvidersLabel", { defaultValue: "Installed sandbox providers:" })}{" "}
            <span className="font-medium text-foreground">
              {discoveredPluginSandboxProviders.map((provider) => provider.displayName).join(", ")}
            </span>
            {t("pages.companyEnvironments.installedProvidersSuffix", { defaultValue: ". These are not adapter types. They back the Sandbox driver for adapters that support sandbox execution." })}
          </div>
        ) : null}

        <div className="overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-xs">
            <caption className="sr-only">{t("pages.companyEnvironments.tableCaption", { defaultValue: "Environment support by adapter" })}</caption>
            <thead className="border-b border-border text-muted-foreground">
              <tr>
                <th className="py-2 pr-3 font-medium">{t("pages.companyEnvironments.colAdapter", { defaultValue: "Adapter" })}</th>
                <th className="px-3 py-2 font-medium">{t("pages.companyEnvironments.colLocal", { defaultValue: "Local" })}</th>
                <th className="px-3 py-2 font-medium">{t("pages.companyEnvironments.colSsh", { defaultValue: "SSH" })}</th>
                {sandboxSupportVisible ? (
                  <th className="px-3 py-2 font-medium">{t("pages.companyEnvironments.colSandboxViaPlugin", { defaultValue: "Sandbox via plugin" })}</th>
                ) : null}
              </tr>
            </thead>
        ...
        <div className="space-y-3">
          {(environments ?? []).length === 0 ? (
            <div className="text-sm text-muted-foreground">{t("pages.companyEnvironments.emptyState", { defaultValue: "No environments saved for this company yet." })}</div>
          ) : (
